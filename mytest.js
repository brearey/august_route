...
axios(config)
  .then((response) => {
    const message = `
    Привет. Вот список названий и времен мероприятий ежегодного августовского совещания работников образования с общей темой “Развитие единой системы образования”:
    ...
    Если я директор школы - какие мероприятия мне посетить? Учитывай время мероприятия. Например если в 09.00-10.00 мероприятие 1, а в 09.00-10.00 мероприятие 2, то я смогу посетить только одно. Напиши только номера мероприятий через запятую.
    `
    // getModelsList(response.data['access_token'])
    // getTokensCount(response.data['access_token'], message)
    requestToGigachat(response.data['access_token'], message)
  })
  .catch((error) => {
    console.log(error)
  });
